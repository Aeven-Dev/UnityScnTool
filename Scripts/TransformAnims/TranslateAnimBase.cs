...
[System.Serializable]
public abstract class TranslateAnimBase : S4Animations
{
    ...
    {
        mKeys.TKey.Add(KeyAt(step * stepDuration, step));
        mKeys.TKey.Add(KeyAt((step + 1) * stepDuration, step + 1));
    }

    void StepEase(int step, TransformKey mKeys)
    {
        mKeys.TKey.Add(KeyAt(step * stepDuration, step));
        mKeys.TKey.Add(KeyAt((step + 1) * stepDuration - 1, step));
    }

    void EaseInEase(int step, TransformKey mKeys)
    {
        mKeys.TKey.Add(KeyAt(step * stepDuration, step));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.5f), step + 0.25f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.75f), step + 0.5f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.875f), step + 0.75f));
        mKeys.TKey.Add(KeyAt((step + 1) * stepDuration, step + 1));
    }
    ...
    {
        mKeys.TKey.Add(KeyAt(step * stepDuration, step));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.125f), step + 0.25f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.25f), step + 0.5f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.5f), step + 0.75f));
        mKeys.TKey.Add(KeyAt((step + 1) * stepDuration, step + 1));
    }
    ...
    {
        mKeys.TKey.Add(KeyAt(step * stepDuration, step));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.125f), step + 0.05f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.25f), step + 0.15f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.5f), step + 0.5f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.75f), step + 0.85f));
        mKeys.TKey.Add(KeyAt(step * stepDuration + (int)(stepDuration * 0.875f), step + 0.95f));
        mKeys.TKey.Add(KeyAt((step + 1) * stepDuration, step + 1));
    }

    TKey KeyAt(int frame, float factor)
    {
        TKey key = new TKey();
        key.frame = frame;
