        ...
        */
        ...
        bool SetTransformToFrame(int frame)
        {
            ...
            foreach (var part in bones.Keys)
            {
                ...
                S4Animation tkd;
                if (copies.TryGetValue(part, out S4Animation anim)) tkd = anim;
                else tkd = bones[part];

                Vector3 newPos = tkd.TransformKeyData.SamplePosition(frame);
                if (part.transform.localPosition != newPos)
                    part.transform.localPosition = newPos;

                Quaternion newRot = tkd.TransformKeyData.SampleRotation(frame);
                if (part.transform.localRotation != newRot)
                    part.transform.localRotation = newRot;

                Vector3 newSca = tkd.TransformKeyData.SampleScale(frame);
                if (part.transform.localScale != newSca)
                    part.transform.localScale = newSca;

                var tr = part.GetComponent<TextureReference>();
				if (tr)
                {
                    float transparency = tkd.TransformKeyData.SampleTransparency(frame);
                    if (tr.transparency != transparency)
        ...
        public void PopulateChannel(Channel channel, VisualElement parent)
        {
            ...
            switch (channel)
            {
                ...
                case Channel.Scale:
                    ...
                    break;
                case Channel.Alpha:
                    for (int i = 0; i < part.AlphaKeys.Count; i++)
                    {
    ...
    public class TickElement : VisualElement
    {
        ...
        public int GetValue()
        {
            switch (type)
            {
                ...
                case 3:
                    return data.AlphaKeys[index].frame;
                default:
                    return 0;
            }
        }
    ...
    class KeyElement : VisualElement
    {
        ...
        float GetValue()
        {
            switch (type)
            {
                ...
                case 3:
                    return data.AlphaKeys[index].Alpha;
                default:
                    return 0;
            }
        }
